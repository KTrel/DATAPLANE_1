...
class Rib:
    ...
    def add_to_rib(self, collector, peer_ip, prefix, ts, as_path):
        ...
        if prefix in self.prefixes:
            ...
            if prefix not in self.rib[collector][peer_ip]:
                self.rib_fd.write(str(collector)+'\t'+str(peer_ip)+'\t'+str(ts)+'\t'+str(prefix)+'\t'+str(as_path)+'\n')
            elif self.rib[collector][peer_ip][prefix] != as_path:
